...
OUT_PNG = os.path.join(os.path.dirname(__file__), "results", "B2_poshold_disturbance_thesis.png")
...
OUT_PNG_XY  = OUT_PNG.replace("_thesis.png", "_thesis_xy.png")
OUT_PNG_ALT = OUT_PNG.replace("_thesis.png", "_thesis_alt.png")
OUT_PNG_ERR = OUT_PNG.replace("_thesis.png", "_thesis_err.png")

# ── Figure 1: XY Trajectory ───────────────────────────────────────────────────
fig1, ax_xy = plt.subplots(figsize=(11, 11))
fig1.subplots_adjust(left=0.14, right=0.97, top=0.90, bottom=0.28)
if pre_rows_xy:
    ax_xy.plot(*zip(*pre_rows_xy),  color="green",  lw=2.0, label="Pre-disturbance")
if dur_rows_xy:
    ax_xy.plot(*zip(*dur_rows_xy),  color="orange", lw=2.0, label="Disturbance → recovery")
if post_rows_xy:
    ax_xy.plot(*zip(*post_rows_xy), color="blue",   lw=1.5, alpha=0.6, label="Post-recovery")
ax_xy.scatter([0], [0], color="red", s=120, zorder=5, label="Setpoint")
theta = np.linspace(0, 2 * np.pi, 200)
ax_xy.plot(0.05 * np.cos(theta), 0.05 * np.sin(theta),
           color="red", linewidth=1.5, linestyle="--", alpha=0.8,
           label="5 cm acceptance boundary [Ref 2, 4]")
ax_xy.set_xlabel("ΔX from setpoint (m)")
ax_xy.set_ylabel("ΔY from setpoint (m)")
ax_xy.set_title("XY Trajectory (relative to setpoint)")
ax_xy.legend(loc='upper center', bbox_to_anchor=(0.5, -0.18), ncol=2, fontsize=16, framealpha=0.95)
ax_xy.grid(True, alpha=0.3)
ax_xy.set_aspect("equal")
fig1.savefig(OUT_PNG_XY, dpi=150, bbox_inches='tight')
plt.close(fig1)
print(f"[B2] XY plot saved: {OUT_PNG_XY}")

# ── Figure 2: Altitude ────────────────────────────────────────────────────────
z_v = [r[3] for r in rows]
fig2, ax_t = plt.subplots(figsize=(14, 10))
fig2.subplots_adjust(left=0.12, right=0.97, top=0.88, bottom=0.22)
ax_t.plot(t_v, z_v, color="purple", linewidth=2.0, label="Altitude (m)")
ax_t.axvline(t_disturbance, color="red", linestyle="--", linewidth=1.5, label="Disturbance")
ax_t.axhspan(0.98, 1.02, color="green", alpha=0.15, label="±2 cm acceptance band [Ref 2]")
ax_t.axhline(1.0, color="green", linewidth=0.8, linestyle=":", alpha=0.6)
ax_t.set_xlabel("Time (s)")
ax_t.set_ylabel("Altitude (m)")
ax_t.set_title("Altitude during Disturbance\nLit. SS altitude error <2 cm [Ref 2]")
ax_t.legend(loc='upper center', bbox_to_anchor=(0.5, -0.14), ncol=3, fontsize=20, framealpha=0.95)
ax_t.grid(True, alpha=0.3)
fig2.savefig(OUT_PNG_ALT, dpi=150, bbox_inches='tight')
plt.close(fig2)
print(f"[B2] Alt plot saved: {OUT_PNG_ALT}")

# ── Figure 3: XY Error over time ──────────────────────────────────────────────
fig3, ax_err = plt.subplots(figsize=(15, 22))
fig3.subplots_adjust(left=0.11, right=0.97, top=0.93, bottom=0.42)
ax_err.plot(t_v, xy_err, color="orange", linewidth=2.0, label="XY distance from setpoint (cm)")
ax_err.plot(t_v, ref_env, color="gray", linewidth=1.5, linestyle=":",
            label=f"Fitted decay envelope (τ={tau_rec:.2f} s)")
ax_err.plot(t_v, lit_fast, color="steelblue", linewidth=1.5, linestyle="--",
            label=f"Lit. fast recovery (τ={TAU_FAST} s) [Ref 3]")
ax_err.plot(t_v, lit_slow, color="navy", linewidth=1.5, linestyle="--",
...
ax_err.fill_between(t_v, _np.nan_to_num(lit_fast, nan=0.0),
                    ...
                    label="Literature recovery band (1–3 s) [Ref 3]")
...
ax_err.set_title(f"EXP-B2: Position Hold Disturbance Rejection  (Fx={IMPULSE_FX} N for {IMPULSE_DUR} s)\n"
                 f"Literature: recovery 1–3 s, SS-RMSE <5 cm [Ref 2, 3]")
ax_err.legend(loc='upper center', bbox_to_anchor=(0.5, -0.08), ncol=1, fontsize=25, framealpha=0.95)
ax_err.grid(True, alpha=0.3)
fig3.savefig(OUT_PNG_ERR, dpi=150)
plt.close(fig3)
print(f"[B2] Err plot saved: {OUT_PNG_ERR}")
